  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_310px]">
      ...
      <div className="space-y-5">
        <Card className="overflow-hidden p-0"><div className="soft-emphasis-panel relative grid aspect-video place-items-center"><div className="absolute h-40 w-40 animate-[spin_8s_linear_infinite] rounded-full border border-[var(--border)]/50" /><span className="z-10 grid h-20 w-20 place-items-center rounded-[28px] bg-[var(--brand)]/15 shadow-[0_0_45px_rgba(109,93,252,.28)]"><Sparkles className="h-9 w-9 text-[var(--accent)]" /></span><div className="absolute bottom-4 text-center"><p className="font-bold">AI 面试官</p><p className="mt-1 text-xs text-white/75">仅在主观题回答后追问</p></div></div><div className="flex items-center justify-between p-4"><div><p className="text-sm font-semibold">语音朗读</p><p className="mt-1 text-xs text-muted-foreground">朗读当前题库原题</p></div><button className="rounded-xl p-2 hover:bg-muted" onClick={() => { setTts(value => !value); window.speechSynthesis?.cancel() }}>{tts ? <Volume2 className="h-4 w-4 text-[var(--accent)]" /> : <VolumeX className="h-4 w-4" />}</button></div><button onClick={() => speak(question.content, true)} className="mx-4 mb-4 flex items-center gap-2 rounded-xl bg-muted px-3 py-2 text-xs font-semibold hover:bg-[var(--accent-soft)] hover:text-[var(--accent)]"><Volume2 className="h-3.5 w-3.5" />重新朗读本题</button></Card>
