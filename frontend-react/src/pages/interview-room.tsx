...
export function InterviewRoom() {
  ...
  async function finish() {
    ...
    setThinking(true)
    try { await request('/v1/interviews/' + id + '/end', { method: 'POST' }); navigate('/candidate/interviews/' + id + '/report') }
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[240px_minmax(0,1fr)_310px]">
      <Card className="h-fit p-3"><div className="flex justify-between px-2 py-2"><strong>面试题目</strong><span className="text-sm text-muted-foreground">{active + 1}/{questions.length}</span></div><div className="mx-2 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full bg-emerald-500" style={{ width: Math.round(((active + 1) / questions.length) * 100) + '%' }} /></div><div className="mt-3 space-y-1">{questions.map((item, index) => <button key={item.interviewQuestionId} onClick={() => setActive(index)} className={'flex w-full gap-3 rounded-xl px-3 py-3 text-left text-sm ' + (index === active ? 'bg-emerald-50 text-emerald-900' : 'hover:bg-muted')}><b className="text-xs">{String(index + 1).padStart(2, '0')}</b><span className="line-clamp-2">{item.content}</span></button>)}</div></Card>
      ...
      <div className="space-y-5">
        <Card className="overflow-hidden p-0"><div className="relative grid aspect-video place-items-center bg-[radial-gradient(circle_at_50%_35%,#1c6c61,#09251f_72%)] text-white"><div className="absolute h-40 w-40 animate-[spin_8s_linear_infinite] rounded-full border border-emerald-200/50" /><span className="z-10 grid h-20 w-20 place-items-center rounded-[28px] bg-emerald-300/15 shadow-[0_0_45px_rgba(52,211,153,.5)]"><Sparkles className="h-9 w-9 text-emerald-100" /></span><div className="absolute bottom-4 text-center"><p className="font-bold">AI 面试官</p><p className="mt-1 text-xs text-emerald-100">仅在主观题回答后追问</p></div></div><div className="flex items-center justify-between p-4"><div><p className="text-sm font-semibold">语音朗读</p><p className="mt-1 text-xs text-muted-foreground">朗读当前题库原题</p></div><button className="rounded-xl p-2 hover:bg-muted" onClick={() => { setTts(value => !value); window.speechSynthesis?.cancel() }}>{tts ? <Volume2 className="h-4 w-4 text-emerald-600" /> : <VolumeX className="h-4 w-4" />}</button></div><button onClick={() => speak(question.content, true)} className="mx-4 mb-4 flex items-center gap-2 rounded-xl bg-muted px-3 py-2 text-xs font-semibold hover:bg-emerald-50 hover:text-emerald-700"><Volume2 className="h-3.5 w-3.5" />重新朗读本题</button></Card>
