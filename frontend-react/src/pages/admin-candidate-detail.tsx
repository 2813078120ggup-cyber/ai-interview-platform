...
export function AdminCandidateDetail() {
  ...
  return <div className="mx-auto max-w-7xl p-6 lg:p-10">
    ...
    <Card className="mt-6 p-0"><div className="border-b border-border p-5"><h2 className="font-bold">历史面试</h2></div><div className="divide-y divide-border">{interviews.map(item => <button key={item.id} onClick={() => navigate(`/admin/interviews/${item.id}/review`)} className="flex w-full items-center justify-between px-5 py-4 text-left hover:bg-muted/50"><div><strong>{item.title}</strong><p className="mt-1 text-xs text-muted-foreground">{dateText(item.scheduledAt)} · {item.duration} 分钟</p></div><Badge tone={item.status === 2 ? 'default' : item.status === 1 ? 'success' : 'info'}>{statusText[item.status]}</Badge></button>)}</div></Card>
  </div>
}
