import { BarChart3, BookOpen, Bot, CalendarDays, LayoutDashboard, LogOut, Moon, Sun, UserRound } from 'lucide-react'
...
export function CandidatePageShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="min-h-screen pl-72">
      <header className="sticky top-0 z-30 flex h-20 items-center justify-end border-b border-border bg-background/82 px-8 backdrop-blur-xl"><div className="flex items-center gap-2"><Button variant="ghost" className="h-10 w-10 rounded-full px-0" onClick={toggleTheme} aria-label={dark ? '切换为浅色模式' : '切换为深色模式'}>{dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button><NotificationCenter role="candidate" /><Button variant="ghost" className="h-10 w-10 rounded-full px-0" onClick={logout} aria-label="退出登录" title="退出登录"><LogOut className="h-4 w-4" /></Button><span title={current?.realName || '候选人'} className="grid h-10 w-10 place-items-center rounded-full bg-[var(--info)] text-sm font-bold text-[var(--info-foreground)]">{initials}</span></div></header>
