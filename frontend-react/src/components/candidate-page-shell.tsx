...
export function CandidatePageShell({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    <aside className="fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-border bg-surface p-4">
      <div className="flex items-center gap-3 px-2 py-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-emerald-600 text-white"><Bot className="h-5 w-5" /></span><div><strong>InterviewOS</strong><p className="text-xs text-muted-foreground">AI 面试评测平台</p></div></div>
      <div className="my-6 rounded-2xl bg-emerald-50 p-3 dark:bg-emerald-400/10"><div className="flex items-center gap-2 text-sm font-semibold text-emerald-700 dark:text-emerald-300"><span className="h-2 w-2 rounded-full bg-emerald-500" />AI 服务正常</div><p className="mt-1 text-xs text-muted-foreground">DeepSeek 面试官在线</p></div>
